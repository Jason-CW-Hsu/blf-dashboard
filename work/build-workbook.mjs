...
{ const s=sheets['業者彙總']; title(s,'受託業者彙總｜依最新月份、基金與區域篩選後排序', 'AB');
  ...
  s.getRange('A8:E8').values=[['國內委外｜業者','委託金額','目前淨資產','市占率（委託金額）','排名']]; header(s,'A8:E8');
  const dm=domestic.filter(r=>r[1]===latestPeriod); const dNames=[...new Set(dm.map(r=>r[5]))].sort((a,b)=>dm.filter(r=>r[5]===b).reduce((t,r)=>t+r[6],0)-dm.filter(r=>r[5]===a).reduce((t,r)=>t+r[6],0));
  s.getRange(`A9:A${8+dNames.length}`).values=dNames.map(x=>[x]);
  for(let r=9;r<9+dNames.length;r++){ s.getRange(`B${r}`).formulas=[[`=SUMIFS('國內委外明細'!$G$4:$G$${3+domestic.length},'國內委外明細'!$F$4:$F$${3+domestic.length},A${r},'國內委外明細'!$A$4:$A$${3+domestic.length},$B$4)`]]; s.getRange(`C${r}`).formulas=[[`=SUMIFS('國內委外明細'!$H$4:$H$${3+domestic.length},'國內委外明細'!$F$4:$F$${3+domestic.length},A${r},'國內委外明細'!$A$4:$A$${3+domestic.length},$B$4)`]]; s.getRange(`D${r}`).formulas=[[`=IFERROR(B${r}/SUM($B$9:$B$${8+dNames.length}),0)`]]; s.getRange(`E${r}`).formulas=[[`=RANK(B${r},$B$9:$B$${8+dNames.length},0)`]]; }
  const os=overseas; const oNames=[...new Set(os.map(r=>r[5]))].sort((a,b)=>os.filter(r=>r[5]===b).reduce((t,r)=>t+r[6],0)-os.filter(r=>r[5]===a).reduce((t,r)=>t+r[6],0)); s.getRange('G8:K8').values=[['國外委外｜業者','委託金額','目前淨資產','市占率（委託金額）','排名']]; header(s,'G8:K8'); s.getRange(`G9:G${8+oNames.length}`).values=oNames.map(x=>[x]);
  for(let r=9;r<9+oNames.length;r++){ s.getRange(`H${r}`).formulas=[[`=SUMIFS('國外委外明細'!$G$4:$G$${3+overseas.length},'國外委外明細'!$F$4:$F$${3+overseas.length},G${r},'國外委外明細'!$A$4:$A$${3+overseas.length},$B$4)`]]; s.getRange(`I${r}`).formulas=[[`=SUMIFS('國外委外明細'!$H$4:$H$${3+overseas.length},'國外委外明細'!$F$4:$F$${3+overseas.length},G${r},'國外委外明細'!$A$4:$A$${3+overseas.length},$B$4)`]]; s.getRange(`J${r}`).formulas=[[`=IFERROR(H${r}/SUM($H$9:$H$${8+oNames.length}),0)`]]; s.getRange(`K${r}`).formulas=[[`=RANK(H${r},$H$9:$H$${8+oNames.length},0)`]]; }
  body(s,`A9:E${8+dNames.length}`); body(s,`G9:K${8+oNames.length}`); s.getRange(`B9:C${8+dNames.length}`).format.numberFormat='#,##0'; s.getRange(`D9:D${8+dNames.length}`).format.numberFormat='0.00%'; s.getRange(`H9:I${8+oNames.length}`).format.numberFormat='#,##0'; s.getRange(`J9:J${8+oNames.length}`).format.numberFormat='0.00%'; s.getRange('A:K').format.autofitColumns(); s.getRange('A:A').format.columnWidth=20; s.getRange('G:G').format.columnWidth=22; s.freezePanes.freezeRows(8);
  ...
  for(let i=0;i<10;i++){s.getRange(`M${4+i}`).formulas=[[`=A${9+i}&" "&TEXT(D${9+i},"0.0%")`]];s.getRange(`N${4+i}`).formulas=[[`=B${9+i}`]];} s.getRange('M14').values=[['其他']];s.getRange('N14').formulas=[[`=SUM(B19:B${8+dNames.length})`]]; for(let r=4;r<=14;r++)s.getRange(`O${r}`).formulas=[[`=N${r}/SUM($N$4:$N$14)`]];
  s.getRange('P3:R3').values=[['國外委外｜前十名+其他','委託金額','市占率']]; header(s,'P3:R3');
  for(let i=0;i<10;i++){s.getRange(`P${4+i}`).formulas=[[`=G${9+i}&" "&TEXT(J${9+i},"0.0%")`]];s.getRange(`Q${4+i}`).formulas=[[`=H${9+i}`]];} s.getRange('P14').values=[['其他']];s.getRange('Q14').formulas=[[`=SUM(H19:H${8+oNames.length})`]]; for(let r=4;r<=14;r++)s.getRange(`R${r}`).formulas=[[`=Q${r}/SUM($Q$4:$Q$14)`]];
  ...
  const dAmount=s.charts.add('bar',s.getRange('M3:N13')); dAmount.titleText='國內委外｜前十名委託金額（業者名稱後附市占率）'; for(let i=0;i<10;i++)dAmount.series.items[0].dataLabelOverrides.add(i).showValue=true; dAmount.setPosition('M16','T31');
  ...
  const oAmount=s.charts.add('bar',s.getRange('P3:Q13')); oAmount.titleText='國外委外｜前十名委託金額（業者名稱後附市占率）'; for(let i=0;i<10;i++)oAmount.series.items[0].dataLabelOverrides.add(i).showValue=true; oAmount.setPosition('M33','T48');
...
function buildDashboard(sheetName, isUsd) {
  ...
  s.getRange('A9:F9').values=[[`資產配置（最新月份，${unit}）`,'新制','舊制','勞保','國保','四大基金合計']]; header(s,'A9:F9'); const cats=['自行運用','委託經營','自行運用-固定收益','自行運用-權益證券','自行運用-另類投資','委託經營-固定收益','委託經營-權益證券','委託經營-另類投資']; s.getRange('A10:A17').values=cats.map(c=>[c]);
  for(let r=10;r<=17;r++){ for(const [col,fund] of [['B','新制'],['C','舊制'],['D','勞工保險'],['E','國民年金']]) s.getRange(`${col}${r}`).formulas=[[`=SUMIFS('月度資產配置'!$E$4:$E$${3+dataRows.length},'月度資產配置'!$A$4:$A$${3+dataRows.length},$A$6,'月度資產配置'!$C$4:$C$${3+dataRows.length},"${fund}",'月度資產配置'!$D$4:$D$${3+dataRows.length},$A${r})/(${divisor})`]]; s.getRange(`F${r}`).formulas=[[`=SUM(B${r}:E${r})`]]; } body(s,'A10:F17'); s.getRange('B10:F17').format.numberFormat=isUsd?'$#,##0.0':'#,##0.0';
