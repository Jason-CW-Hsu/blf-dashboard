...
function managerTotals(rows, section) {
  const items = new Map();
  for (const row of rows.filter(x => x.section === section)) { const name = canonical(row.manager, section); items.set(name, (items.get(name)||0) + row.nav); }
  const total = [...items.values()].reduce((a,b)=>a+b,0);
  return [...items].map(([name,nav]) => ({ name, nav, share: nav / total })).sort((a,b)=>b.nav-a.nav);
}
