...
function parseAssets(pages, fund) {
  ...
  for (const page of pages) for (let i=0; i<page.length; i++) {
    ...
    let item = label;
    if (['固定收益','權益證券','另類投資'].includes(label)) item = `${parent}-國外-${label}`;
    if (item === '國內委託經營') item = '委託經營-國內-權益證券';
...
function parsePensionAssets(pages) {
  ...
  for (const [fund, pageIndex] of [['新制',2],['舊制',3]]) {
    ...
    let parent = '';
    labels.forEach((label,i) => { if(label==='自行運用'||label==='委託經營') parent=label; let item=['固定收益','權益證券','另類投資'].includes(label)?`${parent}-國外-${label}`:label; if(item==='國內委託經營') item='委託經營-國內-權益證券'; results.push({fund,item,amount:pairs[i][0],ratio:pairs[i][1]}); });
  }
  return results;
}
...
function managerTotals(rows, section) {
  ...
  for (const row of rows.filter(x => x.section === section)) {
    ...
    const current = items.get(name) || { nav: 0, change: 0, accounts: 0, accountRows: [] };
    current.nav += row.nav;
    current.change += Number(row.change || 0);
    if (row.accountStatus !== '解約') current.accounts += 1;
    current.accountRows.push({
      fund: row.fund, batch: row.batch, strategy: row.batch, amount: Number(row.amount || 0),
      nav: Number(row.nav || 0), change: Number(row.change || 0), unit: row.unit || '',
      accountStatus: row.accountStatus || '持續', sinceReturn: row.sinceReturn ?? row.sinceRet ?? null,
      target: row.target ?? null,
    });
    items.set(name, current);
  }
  const total = [...items.values()].reduce((a,b)=>a+b.nav,0);
  return [...items].map(([name,stats]) => ({ name, nav: stats.nav, change: stats.change, accounts: stats.accounts, share: stats.nav / total, accountsDetail: stats.accountRows })).sort((a,b)=>b.nav-a.nav);
}
...
// 109 年第二次委託經營（相對報酬）在 PDF 排版上偶爾落入國外區段，實際上屬國內委外。
for (const row of delegated) if (row.batch.replace(/\s/g,'').includes('109年第二次委託經營(相對報酬)')) row.section = '國內';
