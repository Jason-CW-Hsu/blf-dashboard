...
function html(snapshot) {
  ...
  const chartItems=['轉存金融機構','政策性貸款','短期票券','公債、公司債、金融債券及特別股','公債、金融債券、公司債及證券化商品','房屋及土地','政府或公營事業貸款','被保險人貸款','股票及受益憑證投資（含期貨）','自行運用-國外-固定收益','自行運用-國外-權益證券','自行運用-國外-另類投資','委託經營-國內-權益證券','委託經營-國外-固定收益','委託經營-國外-權益證券','委託經營-國外-另類投資'];
  const configs = funds.concat('四大基金合計').map(f => { const a=f==='四大基金合計'? snapshot.assets : snapshot.assets.filter(x=>x.fund===f); const find=item=>a.filter(x=>x.item===item).reduce((s,x)=>s+x.amount,0); return [f, find('合計'), chartItems.map(find)]; });
